...
$user_log_file = "./Logs/User_Errors.log";
$passed = FALSE;
function saveupfile($dog_data_xml, $valid_useridpasswords)
{
 ...
 foreach($valid_useridpasswords as $users)
 {
  foreach($users as $user)
  {
	...
	$xmlstring .= "</users>\n";
 $new_valid_data_file = preg_replace('/[0-9]+/', '', $dog_data_xml); // remove the previous date and time if it exists
...
function retrieve_useridpasswordfile()
{
 ...
 $xmlDoc = new DOMDocument();
	if (file_exists("../xml/dog_applications.xml"))
	{
  $xmlDoc->load("../xml/dog_applications.xml");
...
<html>
 <head><title>Canine Haven Shelter Reservation System</title>
